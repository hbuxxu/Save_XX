...
st18_ErrFlag_TypeDef st18_ErrFlag;
...
{ 
    ...
    /****************************开入通道故障代码初始化***************************/	
		#ifdef USE_11
		un11_Err[0].stSystemErr.ucPass=IN_Brake_ID;
		un11_Err[1].stSystemErr.ucPass=IN_Raise_ID;
		un11_Err[2].stSystemErr.ucPass=IN_LeftStop_ID;
		un11_Err[3].stSystemErr.ucPass=IN_RightStop_ID;
    ...
    /***************************A/D故障代码初始化****************************/	
		#ifdef USE_13
		un13_Err[0].stSystemErr.ucPass=Left_Brake_ID;
		un13_Err[1].stSystemErr.ucPass=Right_Brake_ID;
		un13_Err[1].stSystemErr.ucPass=All_Weight_ID;
    ...
    /***********************流程控制故障代码初始化****************************/		
		#ifdef USE_18
		un18_Err[0].stSystemErr.ucPass=ERR_Test_ID;
		st18_ErrFlag = es18_Err_N;       //无故障
		for(uci=0;uci<Pass_18_Number;uci++)
    {
        un18_Err[uci].stSystemErr.ucErr_Code=18;
				un18_Err[uci].stSystemErr.ucErr_State=st18_ErrFlag;
    }	  
		#endif
    /***************************数据处理故障代码初始化****************************/	
		#ifdef USE_19
...
void SystemErr_W5500Send(void)
{
  ...
  /****************************开入通道故障代码初始化***************************/	
    ...
    for(uci=0;uci<Pass_18_Number;uci++)
    {
				if(un18_Err[uci].stSystemErr.ucErr_State != es18_Err_N)
				{							
							Tx_Buffer[uci+(u8Err_Num*3+4)] = un18_Err[uci].stSystemErr.ucErr_Code;  //故障代码
							Tx_Buffer[uci+(u8Err_Num*3+4)] = un18_Err[uci].stSystemErr.ucPass;  //故障通道号
							Tx_Buffer[uci+(u8Err_Num*3+4)] = un18_Err[uci].stSystemErr.ucErr_State;  //故障类型
...
{
		/****************************开入通道故障代码自检***************************/	
		if(IO_GetInStatus(IN_Brake_ID) == 1)  //制动到位光电
		{
		...
		if(IO_GetInStatus(IN_Raise_ID) == 1)  //举升到位光电
		{
				un11_Err[1].stSystemErr.ucErr_State = es11_Err_1;
		}
		else
		{
				un11_Err[1].stSystemErr.ucErr_State = es11_Err_N;
		}
// 		if(IO_GetInStatus(IN_LeftStop_ID) == 0)  //左停机光电
// 		{
// 				un11_Err[2].stSystemErr.ucErr_State = es11_Err_0;
// 		}
// 		else
// 		{
// 				un11_Err[2].stSystemErr.ucErr_State = es11_Err_N;
// 		}
// 		if(IO_GetInStatus(IN_RightStop_ID) == 0)  //右停机
// 		{
// 				un11_Err[3].stSystemErr.ucErr_State = es11_Err_0;
// 		}
// 		else
// 		{
// 				un11_Err[3].stSystemErr.ucErr_State = es11_Err_N;
// 		}
		/***************************A/D故障代码自检****************************/	
		if(Zero_LeftBrake > Zero_Max)//左制零点电压超范围
		{
				un13_Err[0].stSystemErr.ucErr_State=es13_Err_0;
		}
		if(Zero_RightBrake > Zero_Max)//右制零点电压超范围
		{
				un13_Err[1].stSystemErr.ucErr_State=es13_Err_0;
		}
		if(Zero_AllWeight > Zero_Max) //复重零点电压超范围
		{
				un13_Err[2].stSystemErr.ucErr_State=es13_Err_0;
		}
		/***************************流程控制故障代码自检****************************/	
