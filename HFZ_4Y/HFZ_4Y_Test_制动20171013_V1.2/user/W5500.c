...
//调试使用
unsigned char S3_Port[2];	//端口3的端口号(6666) 调试使用端口 
unsigned char S3_Mode =3;	//端口3的运行模式,0:TCP服务器模式,1:TCP客户端模式,2:UDP(广播)模式
/***************----- 端口的运行状态 -----***************/
unsigned char S3_State =0;	//端口3状态记录,1:端口完成初始化,2端口完成连接(可以正常传输数据) 
/***************----- 端口收发数据的状态 -----***************/
unsigned char S3_Data;		//端口3接收和发送数据的状态,1:端口接收到数据,2:端口发送数据完成 
...
void W5500_GPIO_Configuration(void)
{
	  ...
	  /* 系统复位引脚初始化配置(PE1) */	
		...
		/* Connect EXTI Line9 to PA9    中断引脚设置  */
...
//				if(USER_SystemFlag.A.Flag_Sucess1 == 1)//握手加密成功
//        {
//           USER_SystemFlag.A.Flag_Standalone = 1;  //和数码管联网成功
//        }
//        else
//        {
//            USER_SystemFlag.A.Flag_Standalone = 0;  //和数码管联网成功
//        }
		}
		if(j&IR_DISCON)//在TCP模式下Socket断开连接处理
		{
				...
				USER_SystemFlag.A.Flag_Standalone = 0;  //和数码管联网失败
		}
	...
	if((i & S1_INT) == S1_INT)//Socket1事件处理 
	{
		...
		if(j&IR_CON)//在TCP模式下,Socket1成功连接 
		{
...
// 				USER_SystemFlag.A.Flag_Online = 1;  //和主控联网成功
        ...
        {
            USER_SystemFlag.A.Flag_Online = 0;  //和主控联网失败
  ...
  if((i & S3_INT) == S3_INT)//Socket3事件处理 
	{
		...
		if(j&IR_DISCON)//在TCP模式下Socket断开连接处理
		{
				...
				Socket_Init(3);		//指定Socket(0~7)初始化,初始化端口
...
//		Write_W5500_SOCK_1Byte(i,Sn_TXBUF_SIZE, 0x04);//Socket Tx mempry size=4k  发送设置4k
	}
	for(i=0;i<4;i++)
	{
		Write_W5500_SOCK_1Byte(i,Sn_TXBUF_SIZE, 0x04);//Socket Tx mempry size=4k  发送设置4k
	...
	for(i=4;i<8;i++)
	{
		Write_W5500_SOCK_1Byte(i,Sn_TXBUF_SIZE, 0x00);//Socket Tx mempry size=0k  发送设置0k
	}
  ...
  // 	Write_W5500_1Byte(SIMR,S2_IMR);
	...
	//	Write_W5500_1Byte(SIMR,S1_IMR);//开启接口S1中断
	Write_W5500_SOCK_1Byte(1, Sn_IMR, IMR_SENDOK | IMR_TIMEOUT | IMR_RECV | IMR_DISCON | IMR_CON);//开启socket发送完成，超时，接收数据等中断。
...
void W5500_Socket_Set(void)
{
	...
	if(S1_State==0)//接口1初始化配置                  这里仅配置了socket1.
	{
