...
/* 定时器2定时变量 */		
vu16  TIM2_1ms;  //定时器ms计数  用于测试倒计时时使用
...
//最终结果值
 ...
 uint16_t   Left_StaticWeight,				//左轮静态轴重 结果值
							  ...
							  Left_BrakeSum,           //左制动力最大值  空载制动
								...
								Left_BrakeSub,           //左制动力最大值  空载制动
								Right_BrakeSub,         //右制动力最大值  空载制动
								Left_BrakeSumJJ,       //左制动力最大值  加载制动
								Right_BrakeSumJJ,     //右制动力最大值  加载制动
								Left_BrakeSubJJ,        //左制动力最大值  加载制动
								Right_BrakeSubJJ,      //右制动力最大值  加载制动
								Left_BrakeAve,           //左阻滞
								Right_BrakeAve;         //右阻滞
//左右动态零值								
uint16_t u16Left_BrakeZero, //左动态零值
				 u16Right_BrakeZero;//右动态零值

//停机点数和停机类型
_union_u16_u8_type	Left_StopTime,Right_StopTime;//左右停机时刻
uint8_t Left_StopFlag,Right_StopFlag;					 //左右停机类型标志 ‘1’硬停机  ‘0’软停机
							
uint16_t u16Protect_Limit;           //阻滞力保护阀值								
							
uint16_t u16Left_Data[802],    	//左制动过程数据
							u16Right_Data[802];    //右制动过程数据
uint8_t u8Tran_CurrentZ[3250],				//阻滞上传数据
...
/*************网口发送变量**********************/ 
 ...
 uint8_t con_TranM2[4] = { 0x02, 0x00, 'M', '2'}; 				// 上传踩
 ...
 uint8_t con_TranKACK[6] = { 0x04, 0x00, 'W', 'K',0x00,0x00};  //升级请求回传
 ...
 uint8_t con_TranF[29] = { 29, 0x00, 'F', 103,102,0};    //F指令设备信息
